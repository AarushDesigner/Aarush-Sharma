...
async function loadDashboard() {
  ...
  try {
    ...
    if (statContacts) statContacts.textContent = contactsSnap.size;
        ...
        contactsTable.innerHTML = contactsSnap.docs.map((doc) => {
          ...
          return `
            <tr>
              ...
              <td class="truncate-cell" title="${escapeHtml(c.message)}">${escapeHtml(c.message)}</td>
    ...
    if (contactsTable) contactsTable.innerHTML = `<tr><td colspan="5" class="empty-row">Could not load enquiries.</td></tr>`;
  }
    ...
    if (newsletterTable) {
      if (newsletterSnap.empty) {
        ...
        newsletterTable.innerHTML = newsletterSnap.docs.map((doc) => {
          const n = doc.data();
          return `<tr><td>${escapeHtml(n.email)}</td><td>${formatDate(n.createdAt)}</td></tr>`;
        }).join('');
      }
    }
  } catch (err) {
    console.error('Error loading newsletter list:', err);
    if (newsletterTable) newsletterTable.innerHTML = `<tr><td colspan="2" class="empty-row">Could not load subscribers.</td></tr>`;
  }
}
